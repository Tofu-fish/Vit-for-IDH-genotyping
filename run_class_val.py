...
def get_args():
    ...
    parser.add_argument('--data_set', default='IMNET', choices=['CIFAR', 'IMNET', 'image_folder', 'custom'],
...
def main(args, ds_init):
    ...
    with torch.no_grad():
        ...
        for batch in data_loader_val:
            ...
            with torch.cuda.amp.autocast():
                ...
                output = output.split(images[0].shape[0])
                output = torch.stack([output[0], output[2], output[3]], dim=0)
    ...
    # plt.show()
    print(f"----------pred {s} all {len(y_pred)} acc is {s/len(y_pred)}%")
    print(f'分类0：{a1} True, {a2} False')
    print(f'分类1：{b1} True, {b2} False')
